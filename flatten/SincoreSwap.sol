...
// File contracts/interfaces/ISincoreTradingRoute.sol

pragma solidity 0.5.17;

/**
 * @title Sincore Trading Route
 * @dev The Sincore trading route interface has an standard functions and event
 * for other smart contract to implement to join Sincore Swap as Market Maker.
 */
interface ISincoreTradingRoute {
    /**
...
contract RoutingManagement is Ownable {
    ...
    */
    struct Route {
      string name;
      bool enable;
      ISincoreTradingRoute route;
    }

    event AddedTradingRoute(
        address indexed addedBy,
        string name,
        ISincoreTradingRoute indexed routingAddress,
    ...
    */
    function addTradingRoute(
        string memory _name,
        ISincoreTradingRoute _routingAddress
    )
      public
      onlyOwner
    {
...
contract Partnership is RoutingManagement {
    ...
    * @param partnerIndex Partner or Wallet provider that integrate to Sincore
    ...
    */
    ...
    struct Partner {
      address wallet;       // To receive fee on the Sincore Swap network
    ...
    constructor() public {
        Partner memory partner = Partner(msg.sender, 10, "Sincore"); // 0.1%
        partners[0] = partner;
        emit UpdatePartner(0, msg.sender, 10, "Sincore");
    }
...
// File contracts/SincoreSwap.sol

pragma solidity 0.5.17;

contract SincoreTokenPriviledge is Partnership {
    uint256 public eligibleAmount = 10 ether; // 10 WAD
    IERC20 public SincoreToken;

    event UpdateSincoreToken(IERC20 indexed token);
    event UpdateEligibleAmount(uint256 amount);

    function updateSincoreToken(
        IERC20  token
    )
        public
        onlyOwner
    ...
    function isEligibleForFreeTrade(address user)
        ...
        returns (bool)
    {
        if (address(SincoreToken) == 0x0000000000000000000000000000000000000000) {
            return false;
        }
        return SincoreToken.balanceOf(user) >= eligibleAmount;
    }
}

contract SincoreSwap is SincoreTokenPriviledge, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    */
    ...
    {
        // Load trading route
        ISincoreTradingRoute tradingRoute = tradingRoutes[tradingRouteIndex].route;
        // Trade to route
