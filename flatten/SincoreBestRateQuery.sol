...
// File contracts/bestRate/SincoreBestRateQuery.sol
...
contract SincoreBestRateQuery {
    using SafeMath for uint256;

    uint256 public constant partnerIndex = 0;

    ISincore public Sincore;

    constructor(ISincore _Sincore) public {
        Sincore = _Sincore;
    }
    ...
    returns (
        ...
        // uint256 _amountOut = Sincore.getDestinationReturnAmount(routes[i], src, dest, srcAmount, partnerIndex);
        bytes memory payload = abi.encodeWithSelector(Sincore.getDestinationReturnAmount.selector, route, src, dest, srcAmount, partnerIndex);
        (bool success, bytes memory data) = address(Sincore).staticcall(payload);
        if (success) {
