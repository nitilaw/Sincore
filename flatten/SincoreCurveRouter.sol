...
// File contracts/routes/SincoreCurveRouter.sol
...
contract SincoreCurveRouter is ISincoreTradingRoute, WhitelistedRole, ReentrancyGuard {
    ...
    {
        require(_src != _dest, "SincoreCurveRouter: Destination token can not be source token");
        i = -1;
        j = -1;
        i = _src == busd ? 0 : i;
        i = _src == usdt ? 1 : i;
        ...
        j = _dest == dai ? 2 : j;
        j = _dest == usdc ? 3 : j;
        require(i != -1 && j != -1, "SincoreCurveRouter: Tokens're not supported!");
    }
}
