...
describe('Partnership', () => {
  ...
  it('Should initial data correctly', async () => {
    ...
    expect((await partner.partners(0)).name).to.equal(ethers.utils.formatBytes32String('Sincore').slice(0, 34))
  ...
  it('Should updating existing partner correctly', async function() {
    const fee = 7 // 0.07%
    const name = ethers.utils.formatBytes32String('Sincore').slice(0, 34)
